...
test_ingress_controller() {
    log_info "Testing: NGINX Ingress Controller is running"

    INGRESS_READY=$(kubectl get pods -n ingress-nginx -l app.kubernetes.io/component=controller -o jsonpath='{.items[0].status.phase}' 2>/dev/null || echo "")

    if [ "$INGRESS_READY" = "Running" ]; then
        test_result 0 "NGINX Ingress Controller is running"
    else
        test_result 1 "NGINX Ingress Controller is not running (status: $INGRESS_READY)"
    fi
}
...
main() {
    echo "========================================"
    echo "     kind Application Test Suite"
    echo "========================================"
    echo
